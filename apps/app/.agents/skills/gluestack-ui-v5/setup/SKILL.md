...
bunx gluestack-ui@latest add button

# Add all components at once
bunx gluestack-ui@latest add --all
```
...
bunx gluestack-ui@alpha upgrade
bunx gluestack-ui@alpha add gluestack-ui-provider
bunx gluestack-ui@alpha add --all
```
...
```bash
# Styling engine
bun install nativewind@^5.0.0-preview.2 react-native-css@^3.0.4

# Dev dependencies (Tailwind v4 + PostCSS)
bun install -D tailwindcss@^4.2.0 @tailwindcss/postcss@^4.2.0 postcss@^8.5.0
```
...
```bash
# iOS
cd ios && pod install && bunx expo run:ios
# Android
...
```bash
# Styling engine
bun install uniwind@^1.3.0

# Dev dependency (Tailwind v4)
bun install -D tailwindcss@^4.1.18
```
...
After re-adding the provider via `bunx gluestack-ui@alpha add gluestack-ui-provider`, the UniWind version handles this automatically.
...
`react-native-worklets` version must match the minor of `react-native-reanimated`. For Expo: `bunx expo install react-native-reanimated && bunx expo install --fix`.
...
Ensure `lightningcss` is pinned to exactly `1.30.1` in both `overrides` and `resolutions`, then `rm -rf node_modules && bun install`.
