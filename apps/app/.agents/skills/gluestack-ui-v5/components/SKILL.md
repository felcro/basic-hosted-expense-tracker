...
Gluestack-ui uses a copy-paste approach. Components are copied into your codebase, not installed as bun packages.
...
After re-adding `GluestackUIProvider` via `bunx gluestack-ui@alpha add gluestack-ui-provider`, the provider handles theme switching automatically for your chosen engine.
